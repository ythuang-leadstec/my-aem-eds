  ...
  bannerItemsConfig.forEach((item, index) => {
    ...
    if (item.banner.source && bannerSlide) {
      ...
      if (item.banner.imagePc) {
         ...
         if (cols[0]) moveInstrumentation(cols[0], bannerSlide.querySelector('.pc-banner')?.closest('picture') || bannerSlide.querySelector('.banner__images'));
         if (cols[2]) moveInstrumentation(cols[2], bannerSlide.querySelector('.mb-banner')?.closest('picture') || bannerSlide.querySelector('.banner__images'));

         // Title (Col 4)
         if (cols[4]) moveInstrumentation(cols[4], bannerSlide.querySelector('.banner__title'));
         
         // Description (Col 5)
         if (cols[5]) moveInstrumentation(cols[5], bannerSlide.querySelector('.banner__subtitle'));
         
         // CTA Label (Col 6) / Link (Col 7)
         // Usually put on the button anchor
         if (cols[6]) moveInstrumentation(cols[6], bannerSlide.querySelector('.banner__btn'));
      }
    }
  });
